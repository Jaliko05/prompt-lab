  ...
  return (
    <Card className="border-slate-200 dark:border-slate-800 hover:shadow-md transition-shadow duration-200 bg-white dark:bg-slate-900">
      <CardHeader className="p-4 pb-3">
        <CardTitle className="line-clamp-1 dark:text-slate-100 text-base">{prompt.name}</CardTitle>
      ...
      <CardContent className="space-y-3 p-4 pt-0">
        ...
        {prompt.generation_params && (
          <div className="flex flex-wrap gap-1.5">
            <Badge variant="secondary" className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs">
        ...
        <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 pt-1">
          <div className="flex items-center gap-1">
            ...
            {prompt.last_updated 
              ? format(new Date(prompt.last_updated), "dd/MM/yy", { locale: es })
              : format(new Date(prompt.created_date), "dd/MM/yy", { locale: es })}
          </div>
        </div>
